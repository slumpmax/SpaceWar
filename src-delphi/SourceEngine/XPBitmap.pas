...
type
  TXPDrawMode = (xdmOpaque = 1, xdmAlpha, xdmMono, xdmPalette);
  TXPPutMode = (xpmDefault = 0, xpmOpaque, xpmAlpha, xpmMono, xpmPalette);
  TXPDrawModeEx = (
    xdmxOpaque = 1, xdmxAlpha, xdmxMono, xdmxPalette,
...
//    SrcRect: TRect;
  ...
  protected
    ...
    function DrawTo(gbm: TXPBitmap; px, py: Integer): Boolean; overload; virtual;
    function DrawTo(gbm: TXPBitmap; px, py: Integer; AMode: TXPDrawMode): Boolean; overload; virtual;
    function DrawTo(gbm: TXPBitmap; drect: TRect): Boolean; overload; virtual;
    function DrawTo(gbm: TXPBitmap; drect: TRect; AMode: TXPDrawMode): Boolean; overload; virtual;
  ...
  private
    ...
    FAlphaValue, FDrawAlphaValue: Integer;
    FPalette: TXPPalette;
    function GetCanvas: TCanvas;
    procedure SetAlphaValue(AValue: Integer);
    procedure SetDrawAlphaValue(AValue: Integer);
  ...
  protected
    function DrawTo(ABitmap: TXPBitmap; AX, AY: Integer): Boolean; overload; override;
    function DrawTo(ABitmap: TXPBitmap; AX, AY: Integer; AMode: TXPDrawMode): Boolean; overload; override;
    function DrawTo(ABitmap: TXPBitmap; ARect: TRect): Boolean; overload; override;
    function DrawTo(ABitmap: TXPBitmap; ARect: TRect; AMode: TXPDrawMode): Boolean; overload; override;

    procedure DrawTileTo(gbm: TXPBitmap; AMode: TXPDrawMode; px: Integer = 0; py: Integer = 0); overload;
    procedure DrawTileTo(gbm: TXPBitmap; drect: TRect; AMode: TXPDrawMode; px: Integer = 0; py: Integer = 0); overload;
  ...
  public
    ...
    procedure DrawTo(ACanvas: TCanvas; px, py: Integer; Alpha: Byte = 255); overload;
    procedure DrawTo(ACanvas: TCanvas; ARect: TRect; Alpha: Byte = 255); overload;

    procedure DrawTo(ACanvas: TXPCanvas; px, py: Integer; Alpha: Byte = 255); overload;

    procedure DrawSmoothTo(ACanvas: TCanvas; ARect: TRect);
    ...
    property DrawAlphaValue: Integer read FDrawAlphaValue write SetDrawAlphaValue;
...
begin
  Result := FetchDrawModeEx(DrawMode, FDrawAlphaValue);
end;
...
procedure TXPBitmap.DrawTo(ACanvas: TCanvas; px, py: Integer; Alpha: Byte);
var
...
procedure TXPBitmap.DrawTo(ACanvas: TCanvas; ARect: TRect; Alpha: Byte);
begin
  ACanvas.Lock;
  try
...
procedure TXPBitmap.DrawTo(ACanvas: TXPCanvas; px, py: Integer;
  Alpha: Byte = 255);
begin
  Draw(px - ACanvas.XOrigin, py - ACanvas.YOrigin, ACanvas.Bitmap);
  DrawTo(ACanvas.Canvas, px, py, Alpha);
end;

procedure TXPBitmap.DrawSmoothTo(ACanvas: TCanvas; ARect: TRect);
var
  grp: IGPGraphics;
//  brush: IGPBrush;
  bm: IGPBitmap;
...
begin
  if gbm.PutMode <> xpmDefault then
    Result := ReplaceRect(gbm, srect, TXPDrawMode(gbm.PutMode))
...
procedure TXPBitmap.DrawTileTo(gbm: TXPBitmap; AMode: TXPDrawMode; px: Integer = 0; py: Integer = 0);
begin
  DrawTileTo(gbm, gbm.ClipRect, AMode, px, py);
end;

procedure TXPBitmap.DrawTileTo(gbm: TXPBitmap; drect: TRect; AMode: TXPDrawMode; px: Integer = 0; py: Integer = 0);
var
  nx: Integer;
begin
  if (gbm.FWidth = 0) or (gbm.FHeight = 0) then Exit;
  if CropRect(drect, FDefaultClipRect) then
  begin
    px := drect.Left - AbsoluteMod(px, gbm.FWidth);
    py := drect.Top - AbsoluteMod(py, gbm.FHeight);
    while py < drect.Bottom do
    begin
      nx := px;
      while nx < drect.Right do
      begin
        gbm.Draw(nx, py, Self, AMode);
        nx := nx + gbm.FWidth;
      end;
      py := py + gbm.FHeight;
    end;
...
function TXPBitmap.DrawTo(ABitmap: TXPBitmap; AX, AY: Integer): Boolean;
begin
  if PutMode <> xpmDefault then
    Result := ABitmap.Draw(AX, AY, Self, TXPDrawMode(PutMode))
  else Result := ABitmap.Draw(AX, AY, Self, ABitmap.DrawMode);
end;

function TXPBitmap.DrawTo(ABitmap: TXPBitmap; AX, AY: Integer; AMode: TXPDrawMode): Boolean;
var
  difx, dify, xoffs, yoffs, alpha: Integer;
  ptr1, ptr2: PColor32;
  dmode: TXPDrawModeEx;
  sgbm: TXPBitmap;
  srect: TRect;
begin
  ...
  if (difx <= 0) or (dify <= 0) or (ABitmap.FWidth <= 0) or (ABitmap.FHeight <= 0) then Exit;
  srect.TopLeft := Point(AX, AY);
  srect.BottomRight := Point(AX + difx, AY + dify);
  Result := CropRect(srect, ABitmap.FDrawClipRect, xoffs, yoffs, FlipX, FlipY);
  xoffs := xoffs + FDrawClipRect.Left;
  yoffs := yoffs + FDrawClipRect.Top;
  if Result then
  begin
    alpha := (ABitmap.FDrawAlphaValue * FAlphaValue) div 255;
    if PutMode <> xpmDefault then
      sgbm := Self
    else sgbm := ABitmap;
    dmode := FetchDrawModeEx(Amode, alpha);
    ...
    try
      ...
      begin
        ...
        case dmode of
          ...
          xdmxOpaqueEx: MMXCopyColor(ptr1, ptr2, difx, FlipX, alpha);
          xdmxAlpha: MMXCopyAlpha(ptr1, ptr2, difx, FlipX);
          xdmxAlphaEx: MMXCopyAlpha(ptr1, ptr2, difx, FlipX, alpha);
          xdmxMono: CopyMono(ptr1, ptr2, difx, FlipX, sgbm.MonoColor);
          xdmxMonoEx: CopyMono(ptr1, ptr2, difx, FlipX, sgbm.MonoColor, alpha);
          xdmxPalette: CopyAlphaPalette(ptr1, ptr2, difx, FlipX, sgbm.PaletteTable, sgbm.PaletteCount);
          xdmxPaletteEx: CopyAlphaPalette(ptr1, ptr2, difx, FlipX, sgbm.PaletteTable, sgbm.PaletteCount, alpha);
        end
        else case dmode of
          xdmxOpaque: CopyColor(ptr1, ptr2, difx, FlipX);
          xdmxOpaqueEx: CopyColor(ptr1, ptr2, difx, FlipX, alpha);
          xdmxAlpha: CopyAlpha(ptr1, ptr2, difx, FlipX);
          xdmxAlphaEx: CopyAlpha(ptr1, ptr2, difx, FlipX, alpha);
          xdmxMono: CopyMono(ptr1, ptr2, difx, FlipX, sgbm.MonoColor);
          xdmxMonoEx: CopyMono(ptr1, ptr2, difx, FlipX, sgbm.MonoColor, alpha);
          xdmxPalette: CopyAlphaPalette(ptr1, ptr2, difx, FlipX, sgbm.PaletteTable, sgbm.PaletteCount);
          xdmxPaletteEx: CopyAlphaPalette(ptr1, ptr2, difx, FlipX, sgbm.PaletteTable, sgbm.PaletteCount, alpha);
...
function TXPBitmap.DrawTo(ABitmap: TXPBitmap; ARect: TRect): Boolean;
begin
  if PutMode <> xpmDefault then
    Result := ABitmap.Draw(ARect, Self, TXPDrawMode(PutMode))
  else Result := ABitmap.Draw(ARect, Self, ABitmap.DrawMode);
end;

function TXPBitmap.DrawTo(ABitmap: TXPBitmap; ARect: TRect; AMode: TXPDrawMode): Boolean;
var
  param: TXPDrawParam;
  ptr1, ptr2: PColor32;
  difx, dify, px, py, alpha: Integer;
  dmode: TXPDrawModeEx;
  xbm: TXPBitmap;
begin
    ...
    try
      ...
      begin
        ...
        case dmode of
          ...
          xdmxOpaqueEx: MMXCopyColor(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX, alpha);
          xdmxAlpha: CopyAlpha(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX);
          xdmxAlphaEx: CopyAlpha(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX, alpha);
          xdmxMono: CopyMono(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX, xbm.MonoColor);
          xdmxMonoEx: CopyMono(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX, xbm.MonoColor, alpha);
          ...
          xdmxPaletteEx: CopyAlphaPalette(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX, xbm.PaletteTable, xbm.PaletteCount, alpha);
        end
        else case dmode of
          xdmxOpaque: CopyColor(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX);
          xdmxOpaqueEx: CopyColor(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX, alpha);
          xdmxAlpha: CopyAlpha(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX);
          xdmxAlphaEx: CopyAlpha(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX, alpha);
          xdmxMono: CopyMono(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX, xbm.MonoColor);
          xdmxMonoEx: CopyMono(ptr1, ptr2, param.SrcWidth, param.DestWidth, difx, param.XOffset, param.FlipX, xbm.MonoColor, alpha);
...
begin
  ...
  begin
    vptr := @ScanLine[py][px];
    try
      xmode := FetchDrawModeEx(AMode, FDrawAlphaValue);
      case xmode of
        xdmxOpaque: vptr^ := c32;
        xdmxOpaqueEx: PlotAlpha(vptr^, c32.ChangeAlpha(FDrawAlphaValue));
        xdmxAlpha: PlotAlpha(vptr^, c32);
        xdmxAlphaEx: PlotAlpha(vptr^, c32, FDrawAlphaValue);
        xdmxMono: PlotAlpha(vptr^, GetMonoColor(c32, MonoColor));
        xdmxMonoEx: PlotAlpha(vptr^, GetMonoColor(c32, MonoColor), FDrawAlphaValue);
        xdmxPalette: PlotAlpha(vptr^, GetPalette(c32.BGRA));
        xdmxPaletteEx: PlotAlpha(vptr^, GetPalette(c32.BGRA), FDrawAlphaValue)
      end;
    except
      vptr^ := c32;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      case FetchDrawModeEx(AMode, FDrawAlphaValue) of
        xdmxOpaque: FillColor(vptr, BrushColor, qx);
        xdmxOpaqueEx: FillAlpha(vptr, BrushColor.ChangeAlpha(FDrawAlphaValue), qx);
        xdmxAlpha: FillAlpha(vptr, BrushColor, qx);
        xdmxAlphaEx: FillAlpha(vptr, BrushColor, qx, FDrawAlphaValue);
        xdmxMono: FillAlpha(vptr, GetMonoColor(BrushColor, MonoColor), qx);
        xdmxMonoEx: FillAlpha(vptr, GetMonoColor(BrushColor, MonoColor), qx, FDrawAlphaValue);
        xdmxPalette: FillAlpha(vptr, GetPalette(BrushColor.BGRA), qx);
        xdmxPaletteEx: FillAlpha(vptr, GetPalette(BrushColor.BGRA), qx, FDrawAlphaValue);
      end;
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      case FetchDrawModeEx(AMode, FDrawAlphaValue) of
        ...
        xdmxOpaqueEx: FillAlphaInc(ptr, BrushColor.ChangeAlpha(FDrawAlphaValue), qy, FWidth);
        xdmxAlpha: FillAlphaInc(ptr, BrushColor, qy, FWidth);
        xdmxAlphaEx: FillAlphaInc(ptr, BrushColor, qy, FWidth, FDrawAlphaValue);
        xdmxMono: FillAlphaInc(ptr, GetMonoColor(BrushColor, MonoColor), qy, FWidth);
        xdmxMonoEx: FillAlphaInc(ptr, GetMonoColor(BrushColor, MonoColor), qy, FWidth, FDrawAlphaValue);
        xdmxPalette: FillAlphaInc(ptr, GetPalette(BrushColor.BGRA), qy, FWidth);
        xdmxPaletteEx: FillAlphaInc(ptr, GetPalette(BrushColor.BGRA), qy, FWidth, FDrawAlphaValue);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      dmode := FetchDrawModeEx(AMode, FDrawAlphaValue);
      ...
      begin
        ...
        case dmode of
          ...
          xdmxOpaqueEx: FillAlpha(vptr, BrushColor.ChangeAlpha(FDrawAlphaValue), nx);
          xdmxAlpha: FillAlpha(vptr, BrushColor, nx);
          xdmxAlphaEx: FillAlpha(vptr, BrushColor, nx, FDrawAlphaValue);
          xdmxMono: FillAlpha(vptr, GetMonoColor(BrushColor, MonoColor), nx);
          xdmxMonoEx: FillAlpha(vptr, GetMonoColor(BrushColor, MonoColor), nx, FDrawAlphaValue);
          xdmxPalette: FillAlpha(vptr, GetPalette(BrushColor.BGRA), nx);
          xdmxPaletteEx: FillAlpha(vptr, GetPalette(BrushColor.BGRA), nx, FDrawAlphaValue);
        end;
        Inc(rect.Top);
      end;
    end;
...
begin
  if gbm.PutMode <> xpmDefault then
    gbm.DrawTileTo(Self, ClipRect, TXPDrawMode(gbm.PutMode), px, py)
  else gbm.DrawTileTo(Self, ClipRect, DrawMode, px, py);
end;

procedure TXPBitmap.DrawTile(px, py: Integer; gbm: TXPBitmap; AMode: TXPDrawMode);
begin
  gbm.DrawTileTo(Self, ClipRect, AMode, px, py);
end;

procedure TXPBitmap.DrawTile(px, py: Integer; gbm: TXPBitmap; drect: TRect);
begin
  if gbm.PutMode <> xpmDefault then
    gbm.DrawTileTo(Self, drect, TXPDrawMode(gbm.PutMode), px, py)
  else gbm.DrawTileTo(Self, drect, DrawMode, px, py);
end;

procedure TXPBitmap.DrawTile(px, py: Integer; gbm: TXPBitmap; drect: TRect; AMode: TXPDrawMode);
begin
  gbm.DrawTileTo(Self, drect, AMode, px, py);
end;
...
function TXPGraphic.DrawTo(gbm: TXPBitmap; px, py: Integer;
  AMode: TXPDrawMode): Boolean;
begin
  raise Exception.Create('XPGraphic abstract error.');
end;

function TXPGraphic.DrawTo(gbm: TXPBitmap; px, py: Integer): Boolean;
begin
  raise Exception.Create('XPGraphic abstract error.');
end;

function TXPGraphic.DrawTo(gbm: TXPBitmap; drect: TRect;
  AMode: TXPDrawMode): Boolean;
begin
  raise Exception.Create('XPGraphic abstract error.');
end;

function TXPGraphic.DrawTo(gbm: TXPBitmap; drect: TRect): Boolean;
begin
  raise Exception.Create('XPGraphic abstract error.');
end;
...
procedure TXPBitmap.SetDrawAlphaValue(AValue: Integer);
begin
  FDrawAlphaValue := IntInside(AValue, 0, 255);
end;

...
type
  ...
  TColor24Array = array[Word] of TColor24Entry;
...
function StringIndex(AItem, ALists: string; ADigit: Integer): Integer; overload;
function StringIndex(AItem: string; const ALists: array of string): Integer; overload;
...
procedure DrawBig3X(ADest, ASource: TBitmap);
procedure DrawSmall3X(ADest, ASource: TBitmap);
...
function StrToFieldValue(AValue: string; ADataType: TFieldType; ADefault: Variant): Variant;
function VerifyFieldValue(AValue: Variant; ADataType: TFieldType): Variant;
...
function CurrencyToFloat(AValue: string; ADefault: Double): Double;
...
procedure DrawPictureTo(ACanvas: TCanvas; ARect: TRect; APicture: TPicture; Align: TAlignment = taCenter);
procedure LoadImageFromFile(ACanvas: TCanvas; ARect: TRect; AFileName: string; Align: TAlignment = taCenter); overload;
procedure LoadImageFromFile(ABitmap: TBitmap; AFileName: string); overload;
procedure LoadImageFromFile(ABitmap: TBitmap; ARect: TRect; AFileName: string; Align: TAlignment = taCenter); overload;
procedure LoadImageFromResource(ABitmap: TBitmap; ARect: TRect; AResName, AResType: string; Align: TAlignment = taCenter); overload;
procedure LoadImageFromFile(AImage: TImage; AFileName: string; Align: TAlignment = taCenter); overload;
...
function StringIndex(AItem: string; const ALists: array of string): Integer;
var
  n: Integer;
begin
  Result := -1;
...
function StringIndex(AItem, ALists: string; ADigit: Integer): Integer;
var
  p: Integer;
begin
  Result := -1;
  p := Pos('[' + AItem + ']', ALists);
  if p > 0 then Result := StrToIntDef(Copy(ALists, p - ADigit, ADigit), -1);
end;
...
procedure DrawBig3X(ADest, ASource: TBitmap);
var
  sptr: array[0..2] of PColor24Array;
  dptr: array[0..2] of PColor24Array;
  x, y, ox, sw, sh, r, g, b, n0, n2: Integer;
begin
  sw := ASource.Width;
  sh := ASource.Height;
  for y := 0 to sh - 1 do
  begin
    sptr[1] := ASource.ScanLine[y];
    if y > 0 then
      sptr[0] := ASource.ScanLine[y - 1]
    else sptr[0] := sptr[1];
    if y < sh - 1 then
      sptr[2] := ASource.ScanLine[y + 1]
    else sptr[2] := sptr[1];
    dptr[0] := ADest.ScanLine[y * 3];
    dptr[1] := ADest.ScanLine[y * 3 + 1];
    dptr[2] := ADest.ScanLine[y * 3 + 2];
    for x := 0 to 2 do dptr[x] := ADest.ScanLine[y * 3 + x];
    for x := 0 to sw - 1 do
    begin
      if x > 0 then n0 := -1 else n0 := 0;
      if x < sw - 1 then n2 := 1 else n2 := 0;
      ox := x * 3;

      dptr[1][ox + 1] := sptr[1][1];

      r := (sptr[1][n0].R + sptr[1][1].R * 2) div 3;
      g := (sptr[1][n0].G + sptr[1][1].G * 2) div 3;
      b := (sptr[1][n0].B + sptr[1][1].B * 2) div 3;
      dptr[1][ox].SetRGB(r, g, b);

      r := (sptr[1][n2].R + sptr[1][1].R * 2) div 3;
      g := (sptr[1][n2].G + sptr[1][1].G * 2) div 3;
      b := (sptr[1][n2].B + sptr[1][1].B * 2) div 3;
      dptr[1][ox + 2].SetRGB(r, g, b);

      r := (sptr[0][1].R + sptr[1][1].R * 2) div 3;
      g := (sptr[0][1].G + sptr[1][1].G * 2) div 3;
      b := (sptr[0][1].B + sptr[1][1].B * 2) div 3;
      dptr[0][ox + 1].SetRGB(r, g, b);

      r := (sptr[2][1].R + sptr[1][1].R * 2) div 3;
      g := (sptr[2][1].G + sptr[1][1].G * 2) div 3;
      b := (sptr[2][1].B + sptr[1][1].B * 2) div 3;
      dptr[2][ox + 1].SetRGB(r, g, b);

      r := (sptr[0][n0].R + sptr[1][n0].R * 2 + sptr[0][1].R * 2 + sptr[1][1].R * 4) div 9;
      g := (sptr[0][n0].G + sptr[1][n0].G * 2 + sptr[0][1].G * 2 + sptr[1][1].G * 4) div 9;
      b := (sptr[0][n0].B + sptr[1][n0].B * 2 + sptr[0][1].B * 2 + sptr[1][1].B * 4) div 9;
      dptr[0][ox].SetRGB(r, g, b);

      r := (sptr[0][n2].R + sptr[1][n2].R * 2 + sptr[0][1].R * 2 + sptr[1][1].R * 4) div 9;
      g := (sptr[0][n2].G + sptr[1][n2].G * 2 + sptr[0][1].G * 2 + sptr[1][1].G * 4) div 9;
      b := (sptr[0][n2].B + sptr[1][n2].B * 2 + sptr[0][1].B * 2 + sptr[1][1].B * 4) div 9;
      dptr[0][ox + 2].SetRGB(r, g, b);

      r := (sptr[2][n0].R + sptr[1][n0].R * 2 + sptr[2][1].R * 2 + sptr[1][1].R * 4) div 9;
      g := (sptr[2][n0].G + sptr[1][n0].G * 2 + sptr[2][1].G * 2 + sptr[1][1].G * 4) div 9;
      b := (sptr[2][n0].B + sptr[1][n0].B * 2 + sptr[2][1].B * 2 + sptr[1][1].B * 4) div 9;
      dptr[2][ox].SetRGB(r, g, b);

      r := (sptr[2][n2].R + sptr[1][n2].R * 2 + sptr[2][1].R * 2 + sptr[1][1].R * 4) div 9;
      g := (sptr[2][n2].G + sptr[1][n2].G * 2 + sptr[2][1].G * 2 + sptr[1][1].G * 4) div 9;
      b := (sptr[2][n2].B + sptr[1][n2].B * 2 + sptr[2][1].B * 2 + sptr[1][1].B * 4) div 9;
      dptr[2][ox + 2].SetRGB(r, g, b);
    end;
  end;
end;
...
begin
  ...
  for y := 0 to dh - 1 do
  begin
    dptr := ADest.ScanLine[y];
    for x := 0 to 2 do sptr[x] := ASource.ScanLine[y * 3 + x];
    for x := 0 to dw - 1 do
    begin
      ox := x * 3;
      r := (sptr[1][ox + 1].R * 4
        + sptr[0][ox + 1].R * 2 + sptr[2][ox + 1].R * 2 + sptr[1][ox].R * 2 + sptr[1][ox + 2].R * 2
        + sptr[0][ox].R + sptr[0][ox + 2].R + sptr[2][ox].R + sptr[2][ox + 2].R
      ) div 16;
      g := (sptr[1][ox + 1].G * 4
        + sptr[0][ox + 1].G * 2 + sptr[2][ox + 1].G * 2 + sptr[1][ox].G * 2 + sptr[1][ox + 2].G * 2
        + sptr[0][ox].G + sptr[0][ox + 2].G + sptr[2][ox].G + sptr[2][ox + 2].G
      ) div 16;
      b := (sptr[1][ox + 1].B * 4
        + sptr[0][ox + 1].B * 2 + sptr[2][ox + 1].B * 2 + sptr[1][ox].B * 2 + sptr[1][ox + 2].B * 2
        + sptr[0][ox].B + sptr[0][ox + 2].B + sptr[2][ox].B + sptr[2][ox + 2].B
      ) div 16;
      dptr[x].SetRGB(r, g, b);
    end;
  end;
...
begin
  case ADataType of
    ftBoolean:
    begin
      if Trim(AValue) = '' then ADefault := False;
      Result := StrToBoolDef(AValue, VarToBoolDef(ADefault, False));
    end;
    ftCurrency, ftFloat, ftBCD:
    begin
      if Trim(AValue) = '' then ADefault := 0.0;
      Result := CurrencyToFloat(AValue, VarToFloatDef(ADefault, 0.0));
    end;
    ftInteger:
    begin
      if Trim(AValue) = '' then ADefault := 0;
      Result := Floor(CurrencyToFloat(AValue, VarToIntDef(ADefault, 0)));
    end;
    ftDate, ftTime, ftDateTime, ftTimeStamp:
    begin
      if Trim(AValue) = '' then ADefault := 0.0;
      Result := THStrToDate(AValue, VarToDateTimeDef(ADefault, 0.0));
    end
  else
    Result := AValue;
  end;
...
begin
  case ADataType of
    ftBoolean:
      Result := VarToBoolDef(AValue, False);
    ftCurrency, ftFloat, ftBCD:
      Result := VarToFloatDef(AValue, 0.0);
    ftInteger:
      Result := VarToIntDef(AValue, 0);
    ftDate, ftTime, ftDateTime, ftTimeStamp:
      Result := VarToDateTimeDef(AValue, 0.0);
  else
    Result := AValue;
  end;
end;
...
function CurrencyToFloat(AValue: string; ADefault: Double): Double;
begin
  Result := StrToFloatDef(StringReplace(AValue, ',', '', [rfReplaceAll]), ADefault);
end;
...
begin
  ...
  begin
    ...
    if not TryEncodeDateTime(y - 543, m, d, 0, 0, 0, 0, Result) then Result := ADefault;
  end;
end;
...
asm
  ...
  // edx = mcolor
    ...
    mov   ecx,edx         // ecx: fb rb gb cb
    shr   ecx,24          // ecx: 00 00 00 fb
    inc   eax             // eax: fa +1
    imul  eax,ecx         // edx,eax: (fa + 1) * fb
    shl   eax,16          // eax: ((fa + 1) * fb) << 16
    and   eax,$FF000000   // eax: fab 00 00 00
    and   edx,$00FFFFFF   // edx: 00 rb gb cb
    or    eax,edx         // eax: fab rb gb cb
end;
...
begin
  ...
  w := APicture.Width * lh div APicture.Height;
  if w > lw then
  begin
    w := lw;
    h := APicture.Height * lw div APicture.Width;
  end
  else h := lh;
  ...
  ACanvas.StretchDraw(ARect, APicture.Graphic);
end;

procedure LoadImageFromFile(ACanvas: TCanvas; ARect: TRect; AFileName: string; Align: TAlignment);
var
  pic: TPicture;
begin
  pic := TPicture.Create;
  try
    ...
    DrawPictureTo(ACanvas, ARect, pic, Align);
  finally
    pic.Free;
  end;
end;

procedure LoadImageFromFile(ABitmap: TBitmap; AFileName: string);
begin
  LoadImageFromFile(ABitmap, Rect(0, 0, 0, 0), AFileName, taCenter);
end;

procedure LoadImageFromFile(ABitmap: TBitmap; ARect: TRect; AFileName: string; Align: TAlignment);
var
  w, h: Integer;
  pic: TPicture;
begin
  ...
  try
    ...
    ABitmap.SetSize(w, h);
    DrawPictureTo(ABitmap.Canvas, ARect, pic, Align);
  finally
    pic.Free;
  end;
end;

procedure LoadImageFromResource(ABitmap: TBitmap; ARect: TRect; AResName, AResType: string; Align: TAlignment);
var
...
procedure LoadImageFromFile(AImage: TImage; AFileName: string; Align: TAlignment);
var
  pic: TPicture;
begin
  ...
  try
    pic.LoadFromFile(AFileName);
    AImage.Picture.Bitmap.SetSize(AImage.Width, AImage.Height);
    DrawPictureTo(AImage.Picture.Bitmap.Canvas, AImage.Picture.Bitmap.Canvas.ClipRect, pic, Align);
  finally
    pic.Free;
  end;
end;
